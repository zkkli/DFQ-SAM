...
def test_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    if len(image.shape) == 3:       # 3D数据格式
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                outputs = net(input)
                # print(torch.softmax(outputs, dim=1).shape)  # torch.Size([1, 9, 224, 224])
                # print(outputs.shape)                        # torch.Size([1, 9, 224, 224])
                # exit()
    ...
    # print(net(input).shape)     # torch.Size([1, 9, 224, 224])
    # print(out.shape)            # (224, 224)
    # 这么操作下来，prediction就是一个（224，224）的矩阵，每一个像素的数字代表类别
    # print()
    metric_list = []
...
def inference_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:       # 3D数据格式
        prediction = np.zeros_like(label)
        for ind in range(image.shape[0]):
