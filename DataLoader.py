...
class TestingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_input = {}
        try:
            image = cv2.imread(os.path.join(self.data_path, self.image_paths[index]))
            ori_image = torch.tensor(image).permute(2,0,1)      # 直接加载的图片nparray，转为tensor（原图像）
            # print(image)
            # print(ori_image.shape)      # torch.Size([3, 512, 512])
            # exit()
        ...
        # exit()
        mask_path = os.path.join(self.data_path, self.label_paths[index])
        ori_np_mask = cv2.imread(mask_path, 0)
        ori_np_mask[ori_np_mask!=0] = 1
        # np.set_printoptions(threshold=np.inf)
        # print(ori_np_mask.shape)        # 大小不等
        # print(ori_np_mask)
        # exit()
        ...
        # print(ori_np_mask.shape)
        # print(image.shape)
        # exit()
        

        # transforms = train_transforms(self.image_size, h, w)       # 缩放以匹配encoder，并转为tensor
        transforms = train_transforms2(self.image_size)       # 缩放以匹配encoder，并转为tensor
